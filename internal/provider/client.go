...
// getTemporalClient creates a fresh connection. This is used when a
// retryable error suggests the cached connection may be stale.
//
// The evicted client is NOT closed, because other goroutines may still
// have in-flight RPCs on it (Terraform applies resources in parallel).
// Calling Close() on a shared client causes "the client connection is
// closing" errors on those concurrent RPCs. The old client will be
// garbage-collected once all references are released.
